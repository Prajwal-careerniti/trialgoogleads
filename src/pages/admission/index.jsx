...
export default function Admission() {
  ...
  async function fetchStream() {
    try {
      ...
      const data = GetCookie(`ADMISSION PROCESS-${selectedOption}`);
      if (data) {
        ...
        const response = await axios.get(
          ...
          }/v1/stream/getStream?type=${selectedOption}&streamType=ADMISSION PROCESS`
        );

        // if stream found
        if (response.status === 200) {
          SetCookie(
            `ADMISSION PROCESS-${selectedOption}`,
  ...
  async function fetchStreamProcess(streamId) {
    try {
      const data = GetCookie(`ADMISSION PROCESS-${streamId}`);
      if (data) {
        ...
        const response = await axios.get(
          `${
            import.meta.env.VITE_BACKEND_URL
          }/v1/streamProcess/getStreamProcess?type=ADMISSION PROCESS&streamId=${streamId}`
        );
  ...
  return (
    <div>
      ...
      <section id="admission-img">
        <p id="admission-title">Admission</p>
      </section>
      ...
      <div className="all-courses">
        {streamProcess.map((course) => (
          <div
            ...
            key={course.id}
            onClick={() =>
              navigate(`/admissionInfo?streamProcessId=${course._id}`)
            }
          >
